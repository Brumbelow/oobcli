...
func cmdInit(args []string) error {
    ...
    if err := ensureDir(sessPath); err != nil { return err }
    ...
    if err := writeJSON(metaPath(sessPath), &meta); err != nil { return err }

    fmt.Println("Initialized session:")
    fmt.Printf("  id: %s\n", meta.ID)
    fmt.Printf("  provider: %s\n", meta.Provider)
    if meta.Label != "" { fmt.Printf("  label: %s\n", meta.Label) }
    if meta.Notes != "" { fmt.Printf("  note: %s\n", meta.Notes) }
...
func cmdWatch(args []string) error {
    ...
    if err := fs.Parse(args); err != nil { return err }
    if *sessionID == "" { return errors.New("--session is required") }
    ...
    meta := SessionMeta{}
    if err := readJSON(metaPath(sessPath), &meta); err != nil { return err }
    if *provider != "" { meta.Provider = *provider }

    var outPath string
    if *save != "" { outPath = *save } else { outPath = eventsPath(sessPath) }
    ...
    case "webhook":
        url := ""
        if meta.ProviderKV != nil { url = meta.ProviderKV["webhook_url"] }
        if url == "" {
    ...
    go func() {
        ...
        for scanner.Scan() {
            ...
            if urlStr, domain := parseInteractshLine(line); urlStr != "" || domain != "" {
                if meta.ProviderKV == nil { meta.ProviderKV = map[string]string{} }
                if urlStr != "" { meta.ProviderKV["interactsh_url"] = urlStr }
                if domain != "" { meta.ProviderKV["interactsh_domain"] = domain }
                // Persist meta.json
        ...
        if filter != "" {
            ...
            if err := json.Unmarshal(line, &m); err == nil {
                if p, ok := m["protocol"].(string); ok {
                    if !containsCSV(filter, p) { continue }
                }
            }
        }
        out.Write(line)
...
func watchWebhookSite(inboxURL string, out io.Writer, filter string) error {
    ...
    for {
        ...
        if err != nil { fmt.Fprintln(os.Stderr, "poll error:", err); time.Sleep(5*time.Second); continue }
        ...
        // Parse as {data: []} or []
        var outer struct{ Data []json.RawMessage `json:"data"` }
...
func handleWebhookItem(m map[string]any, seen map[string]struct{}, out io.Writer) {
    ...
    for _, k := range tryKeys {
        if v, ok := m[k]; ok {
            if s, ok := v.(string); ok { id = s; break }
            if f, ok := v.(float64); ok { id = fmt.Sprintf("%.0f", f); break }
        }
    }
    if id == "" {
...
func cmdSendTest(args []string) error {
    ...
    if err != nil { return err }
    sessPath := filepath.Join(sdirRoot, *sessionID)
    if _, err := os.Stat(metaPath(sessPath)); err != nil { return fmt.Errorf("session not found: %s", *sessionID) }
    meta := SessionMeta{}
    if err := readJSON(metaPath(sessPath), &meta); err != nil { return err }
    ...
    switch meta.Provider {
    case "webhook":
        if meta.ProviderKV != nil { base = meta.ProviderKV["webhook_url"] }
        if base == "" { return errors.New("webhook session missing webhook_url; re-init with --webhook-url") }
    case "interactsh":
        base = *targetOverride
        if base == "" {
            if env := os.Getenv("INTERACTSH_URL"); env != "" { base = env }
        }
        if base == "" && meta.ProviderKV != nil {
            // Use captured URL if available
            if v := meta.ProviderKV["interactsh_url"]; v != "" { base = v }
        }
        if base == "" {
    ...
    if reqBody != nil { req.Header.Set("Content-Type", "application/octet-stream") }
    client := &http.Client{ Timeout: 15 * time.Second }
    resp, err := client.Do(req)
    if err != nil { return err }
...
func startWatchBackground(sessPath string, meta SessionMeta, outPath, filter, clientArgs string) error {
    ...
    if meta.Provider != "" { bgArgs = append(bgArgs, "--provider", meta.Provider) }
    if filter != "" { bgArgs = append(bgArgs, "--filter", filter) }
    if outPath != "" { bgArgs = append(bgArgs, "--out", outPath) }
    if clientArgs != "" { bgArgs = append(bgArgs, "--client-args", clientArgs) }
    ...
    cmd.Stdout = f
    cmd.Stderr = f
    if err := cmd.Start(); err != nil { f.Close(); return err }
    pid := cmd.Process.Pid
...
func cmdUp(args []string) error {
    ...
    if *label != "" { initArgs = append(initArgs, "--label", *label) }
    if *note != "" { initArgs = append(initArgs, "--note", *note) }
    if *provider == "webhook" && *webhookURL != "" {
        initArgs = append(initArgs, "--webhook-url", *webhookURL)
    }
    if err := cmdInit(initArgs); err != nil { return err }
    ...
    for _, e := range entries {
        ...
        if err := readJSON(metaPath(p), &m); err == nil {
            if newest.ID == "" || m.CreatedAt.After(newest.CreatedAt) {
                newest = m; newestPath = p
            }
        }
    }
    if newest.ID == "" { return errors.New("failed to locate newly created session") }

    // 2) watch in background
    if err := startWatchBackground(newestPath, newest, eventsPath(newestPath), *filter, *clientArgs); err != nil { return err }
    ...
    if err := cmdSendTest(stArgs); err != nil { fmt.Println("self-test error:", err) }
...
func printEndpoints(meta SessionMeta) {
    ...
    case "interactsh":
        ...
        if base == "" && domain != "" { base = "https://" + domain }
        if base != "" { fmt.Println("HTTP:", base) }
        if domain != "" {
...
func cmdEndpoints(args []string) error {
    ...
    m := SessionMeta{}
    if err := readJSON(metaPath(p), &m); err != nil { return err }
    printEndpoints(m)
    return nil
}
...
func cmdStop(args []string) error {
    ...
    if err != nil { return fmt.Errorf("no watch.pid found for session %s", *sessionID) }
...
func cmdPayloads(args []string) error {
    ...
    case "interactsh":
        ...
        if base == "" && domain != "" { base = "https://" + domain }
        if domain == "" && base != "" {
            if u, err := url.Parse(base); err == nil { domain = u.Hostname() }
        }
        if base == "" && domain == "" { return errors.New("interactsh URL/domain unknown — run 'watch' or 'up' first") }
        fmt.Println("# HTTP")
    ...
    case "webhook":
        base := ""
        if m.ProviderKV != nil { base = m.ProviderKV["webhook_url"] }
        if base == "" { return errors.New("webhook URL unknown — re-init with --webhook-url") }
        fmt.Println("# HTTP")
...
func waitForWebhookTest(meta SessionMeta, testID string, deadline time.Time) bool {
    urlStr := ""
    if meta.ProviderKV != nil { urlStr = meta.ProviderKV["webhook_url"] }
    ...
    for time.Now().Before(deadline) {
        ...
        var outer struct{ Data []map[string]any `json:"data"` }
        var arr []map[string]any
        parsed := false
        if err := json.Unmarshal(b, &outer); err == nil && len(outer.Data) > 0 { arr = outer.Data; parsed = true }
        if !parsed { _ = json.Unmarshal(b, &arr) }
        for _, m := range arr {
            if webhookHasTestID(m, testID) { return true }
        }
    ...
    if hs, ok := m["headers"].(map[string]any); ok {
        for k, v := range hs {
            if strings.EqualFold(k, "X-OOB-Test") {
                if s, ok := v.(string); ok && strings.Contains(s, testID) { return true }
            }
        }
    }
    // Try headers as []map
    if hs, ok := m["headers"].([]any); ok {
        for _, it := range hs {
            if kv, ok := it.(map[string]any); ok {
                ...
                if strings.EqualFold(name, "X-OOB-Test") && strings.Contains(val, testID) { return true }
            }
        }
    }
    // Fallback: search raw content
    for _, k := range []string{"raw", "raw_body", "content", "text"} {
        if s, ok := m[k].(string); ok && strings.Contains(s, testID) { return true }
    }
    return false
}
...
func waitForLocalEvents(eventsFile, testID string, deadline time.Time) bool {
    ...
    for time.Now().Before(deadline) {
        ...
        for sc.Scan() {
            b := sc.Bytes()
            off += int64(len(b)) + 1
            if bytes.Contains(b, []byte(testID)) { f2.Close(); return true }
        }
        f2.Close()
